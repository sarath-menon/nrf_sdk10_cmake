...
#define DEVICE_NAME                                                            \
  "Nordic_Template" /**< Name of device. Will be included in the advertising   \
                       data. */
#define MANUFACTURER_NAME                                                      \
  "NordicSemiconductor" /**< Manufacturer. Will be passed to Device            \
                           Information Service. */
...
static dm_application_instance_t
    m_app_handle; /**< Application identifier allocated by device manager */
...
static void timers_init(void) {
  ...
  APP_TIMER_MAX_TIMERS by one. uint32_t err_code; err_code =
...
static void ble_evt_dispatch(ble_evt_t *p_ble_evt) {
  ...
  /*YOUR_JOB add calls to _on_ble_evt functions from each service your
  application is using ble_xxs_on_ble_evt(&m_xxs, p_ble_evt);
 ...
 * cleared from persistent storage during initialization of the Device Manager.
 */
